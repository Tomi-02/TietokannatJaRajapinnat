const book = {
    bookArray :  [
      ...
      getAllBooks(){
        this.bookArray.forEach(row => {
            console.log(row.id_book + " " + row.name + " " + row.author + " " + row.isbn);
        });
      },

      getOneBook(id){
        const row = this.bookArray.find(row => row.id_book === id);
        console.log(row.id_book + " " + row.name + " " + row.author + " " + row.isbn);
      },
      addBook(id_book, name, author, isbn){
        const book = {'id_book': id_book, 'name': name, 'author': author, 'isbn': isbn};
        this.bookArray.push(book);
        console.log("Book added");
      }
};

book.addBook(16, 'Test', 'TestAuthor', '123456789');
book.getOneBook(1);
book.getAllBooks();
